...
Func Auto($TimeInMs)
	...
	While WinExists("League of Legends (TM) Client")
		While TimerDiff($timer) <= $Clock
			...
			For $j = 1 To 7 
				MouseClickDrag("left", ($pxdifference[0] + Round($gamesz[0] * 0.4714)), ($pxdifference[1] + Round($gamesz[1] * 0.513)), ($pxdifference[0] + Round($gamesz[0] * (0.3 + (0.072 * ($j - 1))))), ($pxdifference[1] + Round($gamesz[1] * 0.61)))
			Next
